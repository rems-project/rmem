...
make fetch_external_deps [UI=...]- try to get some required packages using OPAM
  ...
  Required OCaml packages (missing): $(MISSING_PKGS) $(MISSING_PKG_PINS)
...
fetch_external_deps:
ifneq ($(MISSING_PKGS)$(MISSING_PKG_PINS),)
  ifneq ($(OPAM),)
	$(OPAM) update
    ifneq ($(MISSING_PKGS),)
	$(OPAM) install $(MISSING_PKGS)
    endif
    ifneq ($(MISSING_PKG_PINS),)
	$(foreach p,$(MISSING_PKG_PINS),$(OPAM) pin add $(subst @, ,$(p)) && ) true
    endif
  else
	$(warning missing packages: $(MISSING_PKGS) $(MISSING_PKG_PINS))
