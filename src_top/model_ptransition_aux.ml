...
let p_fuzzy_compare_pt_trans trans1 trans2 = 
  ...
  | PT_Write (ids, (wd, pr, sh), t, _), PT_Write (ids', (wd', pr', sh'), t', _) ->
     cmps [ (fun () -> Pervasives.compare ids ids');
            (fun () -> Pervasives.compare pr pr');
            (fun () -> Pervasives.compare sh sh');
            (fun () -> Pervasives.compare t t');
       ]
  | PT_Write _, _ -> 1
  | _, PT_Write _ -> -1
  ...
  | PT_Read (ids, (rr, t), (w, t2), _), PT_Read (ids', (rr', t'), (w', t2'), _) ->
     cmps [ (fun () -> Pervasives.compare ids ids');
            (fun () -> Pervasives.compare t t'); 
            (fun () -> Pervasives.compare t2 t2'); 
            (fun () -> Pervasives.compare rr rr'); 
            (fun () -> Pervasives.compare w w'); 
       ]
  | PT_Read _, _ -> 1
  | _, PT_Read _ -> -1
