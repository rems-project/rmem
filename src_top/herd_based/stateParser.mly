...
| PERCENTNAME {  $1 }
maybev:
| NUM  { Concrete (Nat_big_num.of_int $1) }
| BIG_NUM { Concrete $1 }
| NAME { Symbolic $1 }
...
atom:
| location {($1,Concrete Nat_big_num.zero)}
| location EQUAL maybev {($1,$3)}

atom_init:
| atom { let x,v = $1 in x,(TyDef,v) }
| NAME location { $2,(Ty $1,Concrete Nat_big_num.zero)}
| ATOMIC NAME location { $3,(Atomic $2,Concrete Nat_big_num.zero)}
| NAME location EQUAL maybev { ($2,(Ty $1,$4))}
| NAME STAR location { ($3,(Pointer $1,Concrete Nat_big_num.zero))}
| NAME STAR location EQUAL amperopt maybev { ($3,(Pointer $1,$6))}
| STAR location { ($2,(TyDefPointer,Concrete Nat_big_num.zero))}
| STAR location EQUAL amperopt maybev { ($2,(TyDefPointer,$5))}
| NAME NAME LBRK NUM RBRK { (Location_global (Symbolic $2),(TyArray ($1,$4),Concrete Nat_big_num.zero)) }
